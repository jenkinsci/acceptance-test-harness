...
public abstract class TSRSlaveController extends CapybaraPortingLayer implements Closeable, AutoCleaned {
     ...
     *
     * <p>
     * Since the actual launch of slave can take some time and it often involves a busy loop until
     * the slave gets fully launched, this method returns {@link Future} and it can return before the
     * slave is fully connected.
     *
     * <p>
     * However, this does not mean the setup process can run entirely in another thread, as {@link WebDriver}
     * do not support concurrent use by multiple threads.
     *
     * <p>
     * For example, SSH slaves might synchronously interact with Jenkins to create a slave, and let Jenkins
     * begin connecting to it, but this method would return without waiting for the slave to fully come online.
     * Then later when {@link Future#get()} method is invoked, it'll check back the slave status and block
     * until the slave becomes online.
     *
     * <p>
     * This design improves the speed of connecting multiple slaves.
     *
     * TODO: for EC2 based providers where there's also another initial delay of allocating a new machine,
     * this abstraction doesn't hide all the latencies sufficiently.
     *
     * <p>
     ...
     *
     * <p>
     * Most notably JNLP slaves can control their own lifecycles, and for those slaves
     * this method lets you disconnect this slave. A stopped slave can be later reconnected
     * via {@link #start()}.
     */
    public void stop() {
