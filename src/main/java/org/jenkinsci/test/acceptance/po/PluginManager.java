...
public class PluginManager extends ContainerPageObject {
    ...
    public boolean installPlugins(final PluginSpec... specs) throws UnableToResolveDependencies, IOException {
        ...
        if (uploadPlugins) {
            ...
            try {
                driver.manage().timeouts().pageLoadTimeout(time.seconds(60), TimeUnit.SECONDS);
                visit("available");
            } finally {
                driver.manage().timeouts().pageLoadTimeout(time.seconds(FallbackConfig.PAGE_LOAD_TIMEOUT), TimeUnit.SECONDS);
            }
            // End JENKINS-50790
