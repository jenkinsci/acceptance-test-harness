    ...
    node('maven-11') {
      ...
      withCredentials([string(credentialsId: 'launchable-jenkins-acceptance-test-harness', variable: 'LAUNCHABLE_TOKEN')]) {
        axes.values().combinations {
          ...
          subsetIds["${jenkinsVersion}-${platform}-jdk${jdk}-${browser}"] = sh(returnStdout: true, script: "launchable subset ${isSubset ? '' : '--observation '}--session ${session} --target 30% --get-tests-from-previous-sessions --split maven").trim()
        }
      }
    }
  }
