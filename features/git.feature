Feature:  Allow use of Git as a build SCM
  ...
  Scenario: Checkout branch from Git repository
    ...
    And I add a script build step to run "if [ `git rev-parse selenium_test_branch` = `git rev-parse HEAD` ]; then exit 0; fi; exit 1"
    And I save the job
    And I build the job
    Then the build should succeed
  ...
  Scenario: Simple checkout from Git repository
    ...
    And I add a script build step to run "if [ ! -d selenium_test_dir ]; then exit 1; fi; cd selenium_test_dir; git remote -v"
    And I save the job
    And I build the job
    Then the build should succeed
    And I should see console output matching "origin git://github.com/jenkinsci/git-plugin.git"
